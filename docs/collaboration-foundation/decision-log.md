...
| DL-017 | 2026-07-15 | Adopt reviewed Pages Wrangler source control with complete local/preview/production non-secret variables, exact-disabled Collaboration, generated runtime/binding types, and no remote bindings. | Implemented; first-deployment evidence pending | Product Owner + Technical Lead + Operations + Security + Senior QA | The first deployment may add only `nodejs_compat` and four reviewed variable names; any other live diff triggers rollback. |
