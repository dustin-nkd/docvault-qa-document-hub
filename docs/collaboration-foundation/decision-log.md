...
| DL-022 | 2026-07-16 | Make the shared `npm run check` a release-blocking Cloudflare gate, enforce a hashed runtime-only `_site` allowlist, compile/inspect Pages Functions with exact `/api/v1/*` routing, and rehearse rollback by reading a pinned compatible commit/deployment without mutating production. | Implemented and locally verified | Technical Lead + Security + Operations + Senior QA | `CF-P1-008` local artifact, CI-order, negative leak, Functions, startup-budget, and read-only rollback gates passed. Production deployment/smoke evidence remains pending. |
