...
| Checkpoint | Day 2 - Gate G1 |
| Status | Draft for cross-functional approval; no runtime implementation authorized |
...
| T17 | E/I | Credential documents enter collaboration via create, copy, import, batch, category mutation, or malformed ciphertext metadata. | 4 | 5 | 20 Critical | P1 | P: client warning plus server category/schema enforcement on every path; no automatic migration; explicit target confirmation. D: bypass matrix and storage inspection. R: quarantine/delete prohibited collaboration record per incident policy; rotate exposed credentials. | Low; Product Owner / Technical Lead | CF-DOC-004, CF-ISO-001-003, AB-11 |
