...
The isolated adapter uses the exact GitHub token and authenticated-user endpoints, POST form encoding, PKCE verifier, exact callback URI, JSON media type, redirect rejection, a 5-second request timeout, and an 8-second overall provider budget. Token exchange is never retried. Identity lookup retries once only for 429/502/503/504 with delay capped at one second. Response bodies are streamed into a 16 KiB hard limit.
