...
test('CI blocks deployment until all Cloudflare, artifact, and browser gates pass', () => {
    ...
    assert.throws(() => validateCloudflareCiBoundary(withoutWorkspaceKeys, workflow));
    const withoutRotation = structuredClone(packageJson);
    withoutRotation.scripts['check:cloudflare'] = withoutRotation.scripts['check:cloudflare']
        .replace(' && npm run cf:phase5:rotation:check', '');
    assert.throws(() => validateCloudflareCiBoundary(withoutRotation, workflow));
    const withoutPreviewKey = structuredClone(packageJson);
    withoutPreviewKey.scripts['check:cloudflare'] = withoutPreviewKey.scripts['check:cloudflare']
        .replace(' && npm run cf:phase5:preview-key:check', '');
    assert.throws(() => validateCloudflareCiBoundary(withoutPreviewKey, workflow));
    ...
    const withoutPhase4PreviewApi = structuredClone(packageJson);
    withoutPhase4PreviewApi.scripts['check:cloudflare'] = withoutPhase4PreviewApi.scripts['check:cloudflare']
        .replace(' && npm run cf:phase4:preview-api:check', '');
    assert.throws(() => validateCloudflareCiBoundary(withoutPhase4PreviewApi, workflow));
    const withoutPhase4Exit = structuredClone(packageJson);
    withoutPhase4Exit.scripts['check:cloudflare'] = withoutPhase4Exit.scripts['check:cloudflare']
        .replace(' && npm run cf:phase4:exit:check', '');
    assert.throws(() => validateCloudflareCiBoundary(withoutPhase4Exit, workflow));
    const withoutPhase5Sprint = structuredClone(packageJson);
    withoutPhase5Sprint.scripts['check:cloudflare'] = withoutPhase5Sprint.scripts['check:cloudflare']
        .replace(' && npm run cf:phase5:sprint:check', '');
    assert.throws(() => validateCloudflareCiBoundary(withoutPhase5Sprint, workflow));
