...
it('isolates parallel worker A storage', async () => {
    await env.COLLAB_DB.prepare(
        'INSERT INTO harness_records (id, value, created_at) VALUES (?, ?, ?)'
    ).bind('parallel-shared-key', 'worker-a', 201).run();
    const snapshot = await snapshotHarnessDatabase(env.COLLAB_DB);
    expect(snapshot.count).toBe(2);
    expect(snapshot.records.find(record => record.id === 'parallel-shared-key')?.value).toBe('worker-a');
});
