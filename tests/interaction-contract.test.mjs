...
test('the Bauhaus UI style switcher and lock screen contracts remain intact', () => {
    ...
    assert.match(css, /\[data-ui-style=["']bauhaus["']\]\s*#viewer-container/, 'style.css must style viewer-container for Bauhaus mode');
    assert.match(css, /\[data-ui-style=["']bauhaus["']\]\s*\.bug-triage-card/, 'style.css must style bug-triage-card for Bauhaus mode');

    assert.match(css, /\[data-ui-style=["']bauhaus["']\]\s*#modal\s+\[role=["']dialog["']\]/, 'style.css must style modal dialog for Bauhaus mode');
    assert.match(css, /\[data-ui-style=["']bauhaus["']\]\s*\.tpl-card/, 'style.css must style template picker cards for Bauhaus mode');
    assert.match(css, /\[data-ui-style=["']bauhaus["']\]\s*#editor-container/, 'style.css must style editor-container for Bauhaus mode');
    assert.match(css, /\[data-ui-style=["']bauhaus["']\]\s*\.tc-step-row/, 'style.css must style tc-step-row for Bauhaus mode');

    assert.match(css, /\[data-ui-style=["']bauhaus["']\]\s*\.focus-hero\s+h3/, 'style.css must style focus hero for Bauhaus mode');
    assert.match(css, /\[data-ui-style=["']bauhaus["']\]\s*\.focus-tab/, 'style.css must style focus tab for Bauhaus mode');
    assert.match(css, /\[data-ui-style=["']bauhaus["']\]\s*\.focus-group/, 'style.css must style focus group for Bauhaus mode');
    assert.match(css, /\[data-ui-style=["']bauhaus["']\]\s*\.trace-summary-item/, 'style.css must style trace summary for Bauhaus mode');
    assert.match(css, /\[data-ui-style=["']bauhaus["']\]\s*\.impact-section/, 'style.css must style api impact section for Bauhaus mode');
    assert.match(css, /\[data-ui-style=["']bauhaus["']\]\s*\.trace-table-wrap/, 'style.css must style traceability table for Bauhaus mode');

    assert.match(css, /\[data-ui-style=["']bauhaus["']\]\s*\.act-day/, 'style.css must style activity day separator for Bauhaus mode');
    assert.match(css, /\[data-ui-style=["']bauhaus["']\]\s*\.act-list/, 'style.css must style activity list for Bauhaus mode');
    assert.match(css, /\[data-ui-style=["']bauhaus["']\]\s*\.act-title/, 'style.css must style activity item title for Bauhaus mode');
    assert.match(css, /\[data-ui-style=["']bauhaus["']\]\s*\.ui-state/, 'style.css must style ui-state empty container for Bauhaus mode');
