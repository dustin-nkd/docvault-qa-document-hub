    ...
    const focusItem = item => {
        ...
        const lifecycle = item.workflowStatus === 'snoozed'
            ...
            : item.workflowStatus === 'done'
                ...
                : '';
        const primaryConfig = item.workflowStatus === 'active'
            ? { action: 'completeFocusItem', className: 'focus-workflow-action is-done', title: t('focusMarkDone'), icon: 'fa-solid fa-check', label: t('focusDone') }
            : item.workflowStatus === 'snoozed'
                ? { action: 'unsnoozeFocusItem', className: 'focus-workflow-action', title: t('focusUnsnooze'), icon: 'fa-solid fa-bell', label: t('focusUnsnooze') }
                : { action: 'reopenFocusItem', className: 'focus-workflow-action', title: t('focusReopen'), icon: 'fa-solid fa-rotate-left', label: t('focusReopen') };
        const primaryAction = renderActionButton({ ...primaryConfig, args: [item.doc.id, item.signalKey] });
        const manage = item.workflowStatus !== 'done'
            ? renderActionButton({
                className: 'focus-workflow-action',
                action: 'showFocusWorkflowModal',
                args: [item.doc.id, item.signalKey],
                title: t('focusManage'),
...
// Trend calculations, SVG chart primitives, and trend markup are defined in render-trends.js.
