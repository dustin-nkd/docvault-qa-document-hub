...
export function validatePhase3SprintPlan({ manifest, sprintSource, traceability, threatModel, riskRegister, wrangler }) {
    ...
    assert(manifest.authorization?.gate === 'P3-G1'
        && manifest.authorization.decision === 'APPROVED'
        && manifest.authorization.authorized_story === 'CF-P3-002', 'P3-G1 authorization drifted');
    ...
    for (const story of stories) {
        assert(story.status === (['CF-P3-001', 'CF-P3-002'].includes(story.id) ? 'PASS' : 'PLANNED')
    ...
    assert(/^Status: \*\*ACTIVE — `CF-P3-002` PASS; awaiting Product Owner approval at Gate P3-G2\*\*$/m.test(sprintSource), 'Sprint document status drifted');
