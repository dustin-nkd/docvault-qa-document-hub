...
export function validatePhase3SprintPlan({ manifest, sprintSource, traceability, threatModel, riskRegister, wrangler }) {
    ...
    assert(manifest.authorization?.gate === 'P3-G4B'
        && manifest.authorization.decision === 'APPROVED'
        && manifest.authorization.authorized_story === 'CF-P3-008', 'P3-G4B authorization drifted');
    ...
    for (const story of stories) {
        assert(story.status === (['CF-P3-001', 'CF-P3-002', 'CF-P3-003', 'CF-P3-004', 'CF-P3-005', 'CF-P3-006', 'CF-P3-007', 'CF-P3-008'].includes(story.id) ? 'PASS' : 'PLANNED')
    ...
    assert(sprintSource.includes('`CF-P3-008` PASS; awaiting Product Owner approval at Gate P3-G4A'), 'Sprint document status drifted');
