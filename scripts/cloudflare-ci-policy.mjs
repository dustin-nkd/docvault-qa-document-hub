...
export function validateCloudflareCiBoundary(packageJson, workflow) {
    ...
    assert(scripts['check:cloudflare'] === 'npm run cf:toolchain:check && npm run cf:config:check && npm run cf:types:check && npm run cf:burst:config:check && npm run cf:burst:types:check && npm run cf:burst:build && npm run test:collab:unit && npm run cf:test && npm run cf:rollback:rehearse && npm run cf:pages:dry-run && npm run cf:phase1:check && npm run cf:phase2:schema:check && npm run cf:phase2:migrations:check && npm run cf:phase2:readiness:check && npm run cf:phase2:persistence:check && npm run cf:phase2:recipes:check && npm run cf:phase2:quality:check && npm run cf:phase2:preview:check && npm run cf:phase2:recovery:check && npm run cf:phase2:exit:check && npm run cf:phase3:sprint:check && npm run cf:phase3:contract:check && npm run cf:phase3:primitives:check && npm run cf:phase3:oauth:check && npm run cf:phase3:callback:check && npm run cf:phase3:session:check && npm run cf:phase3:request-policy:check && npm run cf:phase3:abuse:check && npm run cf:phase3:preview-identity:check && npm run cf:phase3:exit:check && npm run cf:phase4:contract:check && npm run cf:phase4:workspace:check && npm run cf:phase4:rbac:check && npm run cf:phase4:invitations:check && npm run cf:phase4:memberships:check && npm run cf:phase4:audit:check && npm run cf:phase4:preview-api:check && npm run cf:phase4:exit:check', 'The Cloudflare release gate is incomplete or out of order');
