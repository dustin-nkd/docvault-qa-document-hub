const INTERNAL_PATH = '/v1/limit';
const MAXIMUM_BODY_BYTES = 96;
...
async function bodyKey(request: Request): Promise<string | null> {
    ...
    if (length !== null && (!/^\d{1,3}$/.test(length) || Number(length) > MAXIMUM_BODY_BYTES)) return null;
    const text = await request.text();
    if (new TextEncoder().encode(text).byteLength > MAXIMUM_BODY_BYTES) return null;
    let value: unknown;
...
export async function handleIdentityBurst(request: Request, limiter: RateLimit): Promise<Response> {
    ...
    if (request.method !== 'POST' || url.pathname !== INTERNAL_PATH || url.search !== '') {
